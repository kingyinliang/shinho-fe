        ...
        case String:
          if (/^(?:\d+(y|m|d|h|min|s))$/i.test(expireTimes as string)) {
            ...
            switch ((expireTimes as string).replace(/^(?:\d+)(y|m|d|h|min|s)$/i, '$1').toLowerCase()) {
              // Frequency sorting
              case 'm':
                _expires = '; max-age=' + Number(_expireTime) * 2592000
                break // 60 * 60 * 24 * 30
              case 'd':
                _expires = '; max-age=' + Number(_expireTime) * 86400
                break // 60 * 60 * 24
              case 'h':
                _expires = '; max-age=' + Number(_expireTime) * 3600
                break // 60 * 60
              case 'min':
                _expires = '; max-age=' + Number(_expireTime) * 60
                break // 60
              case 's':
                _expires = '; max-age=' + _expireTime
                break
              case 'y':
                _expires = '; max-age=' + Number(_expireTime) * 31104000
